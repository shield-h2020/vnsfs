...
status_file = "l3filtercryptomining_status.log"


@when_not('l3filtercryptomining.configured')
def not_configured():
...
def config_changed():
    try:
        ...
        set_flag("l3filtercryptomining.configured")
...
@when('l3filtercryptomining.configured')
@when('actions.start')
def start():
...
def ssh_call(action_name, cmd):
    try:
        ...
        action_fail(
            "command failed: {}, errors: {}".format(e, str(e)))
    else:
...
def curl_call(action_name, path, method, headers={}, data=""):
    ...
    try:
        ...
        curl_url = "http://{}:{}{}".format(
            ...
            rest_api_port,
            path)
        print("curl_call: URL is " + curl_url +
