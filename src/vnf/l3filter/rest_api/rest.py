...
class MethodRequest(urllib2.Request):
    ...
    def get_method(self, *args, **kwargs):
        return self._method if self._method is not None else \
                urllib2.Request.get_method(self, *args, **kwargs)


class RESTRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    def __init__(self, *args, **kwargs):
        self.routes = routes.get_all()
    ...
    def get_payload(self):
        ...
        m = re.search('(<mspl-set.+?mspl-set>)', payload)
        if m:
            payload = m.group(1)
        else:
            payload = None
...
def rest_server(port):
    ...
    print '[DEBUG] Starting HTTP server at port %d' % port
    try:
